...
function buildSharedRules(schemaText: string): string {
  return [
    ...
    '- renderChart — visualize one runSql result by its resultId and column names.',
    '',
    ...
    '   - Pass the exact resultId from runSql plus labelColumn and valueColumn.',
    '   - Never copy, rewrite, aggregate, or invent chart data. The tool resolves',
    '     values from the selected SQL result. If it rejects the result as too large',
    '     or non-numeric, run one focused aggregate query and chart that new result.',
    ...
    '- For a data-quality question, prefer one aggregate query containing COUNT(*) plus the',
    '  requested per-issue and deduplicated counts. After totals are available, use at most one',
    '  follow-up detail query for representative rows, then answer. Do not run optional value',
    '  distributions, duplicate scans, or entity-matching checks unless the user asked for them.',
...
const BASE_VARIANTS = {
  ...
  v3: [
    ...
    '→ renderChart({ resultId: "query_1", chartType: "bar", title: "Top 5 Products by Revenue", labelColumn: "product_name", valueColumn: "total_revenue", xAxis: "Product", yAxis: "Revenue" })',
    '→ "营收最高的 5 个产品是……"',
    '',
    'User: "2025 年每月销售趋势"',
    ...
    '→ renderChart({ resultId: "query_1", chartType: "line", labelColumn: "month", valueColumn: "monthly_sales", xAxis: "Month", yAxis: "Sales", ... })',
    '→ "2025 年销售额整体呈上升趋势……"',
  ].join('\n'),
